...
const onygheriCommand = require('./commands/اونيغيري');
const fadeyaCommand = require('./commands/fadeya');
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        // ========== إضافة تفاعل عشوائي على كل رسالة (إذا كانت المحادثة مفعلة) ==========
        ...
        // Check owner status for owner commands
         if (isOwnerCommand && userMessage !== '.اونيغيري') {
    ...
    break;
case userMessage === '.اونيغيري':
    await onygheriCommand.execute(sock, message, senderIsOwnerOrSudo);
    commandExecuted = true;
    break;
    case userMessage.startsWith('.فعالية'):
    await fadeyaCommand(sock, chatId, message);
    commandExecuted = true;
    break;
            default:
                if (isGroup) {
        ...
        // تم إزالة استدعاء addCommandReaction من هنا لأنه أصبح في بداية الدالة
        // if (userMessage.startsWith('.')) {
        //     await addCommandReaction(sock, message);
        // }
    } catch (error) {
